...
char* indexes(const char* path)
{
    ...
    tmp_path = malloc(strlen(path) + 10);
    strcpy(tmp_path, path);
    strcat(tmp_path, "index.php");
    ...
    tmp_path = malloc(strlen(path) + 11);
    strcpy(tmp_path, path);
...
const char* lookup(const char* path)
{
    ...
    if(i == 0 || i < strlen(path) - 6)
        return NULL;
    
    i++;
    char* ext = malloc(strlen(path) - i);
    memcpy(ext, &path[i], (strlen(path)-i)*sizeof(*path));
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    char cur_method[4] = "GET";
    char* method = malloc(first_space + 1);
    memcpy(method, &line[0], first_space);
    method[first_space] = '\0';
    if(strcasecmp(method, cur_method) != 0) {
        error(405);
        return false;
    }
    free(method);
    ...
    char* http = malloc(9);
    memcpy(http, &line[strlen(line) - 10], 8);
    http[8] = '\0';
    ...
    if(question_mark_index == 0) {
        memcpy(abs_path, &line[4], strlen(line) - 5 - 10);
        abs_path[strlen(line) - 5 - 10] = '\0';
    } else {
        memcpy(abs_path, &line[4], question_mark_index - 4);
        abs_path[question_mark_index - 4] = '\0';
    }
    
    if(question_mark_index == 0 || (line[question_mark_index + 2] == 'H' && line[question_mark_index + 1] == ' ')) {
        query[0] = '\0';
    } else {
        memcpy(query, &line[question_mark_index + 1], strlen(line) - question_mark_index - 12);
        query[strlen(line) - question_mark_index - 10 + 1] = '\0';
    }
